...
// Login component for handling Spotify login and fetching high scores
const Login = ({ onSpotifyLogin }) => {
  const [highScores, setHighScores] = useState([]);

   // Effect to fetch top scores from the server when the component mounts
  useEffect(() => {
    const fetchTopScores = async () => {
      try {
        // Fetch top scores from the API
        const response = await axios.get(
          `${process.env.REACT_APP_SERVER_URL}/api/top-scores`
        );
        setHighScores(response.data);
      } catch (err) {
        console.error("Unable to fetch high scores");
      }
    };
  ...
  return (
    <div className="login-content">
      ...
      <div className="lyrics-section-login">
        <div className="playing-now-login">
          <div className="init-btn">
            <button onClick={onSpotifyLogin}>Initialize Game</button>
          </div>
